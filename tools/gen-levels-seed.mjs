...
const rows = LEVELS.map((l) => {
  ...
  // chapter：守塔成就「全破」一章一個、換章要整章打完，伺服器要知道每關是哪一章
  // bonus：首次通關的金幣。後兩章不照編號算（第 85 關會變 890），見 core/progress.ts
  return { id: l.id, no: l.no, name: l.name, minCorrect: Math.ceil(enemies * 0.5), boss: !!l.isBoss,
    chapter: l.chapter, bonus: l.bonus }
})
if (rows.length !== 85) throw new Error('關卡數不是 85，是不是改了？' + rows.length)
...
${rows.map((r) => `  ('${r.id}', ${r.no}, '${r.name}', ${r.minCorrect}, ${r.boss}, ${r.chapter}, ${r.bonus})`).join(',\n')}
