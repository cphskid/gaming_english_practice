...
function series(meRate, foeRate, rules, n = 24) {
  const out = { wins: 0, draws: 0, secs: 0, push: 0, pinned: 0, early: 0, touched: 0 }
  for (let i = 0; i < n; i++) {
  ...
  return {
    ...
    pinned: out.pinned / n, earlyRate: out.early / n, touchRate: out.touched / n,
  }
}
...
// 4. 但不管怎樣都不准把落後的孩子的城堡打爆——他會直接不玩了。
for (const r of rows) {
  if (r.earlyRate > 0) fails.push(`${r.label} 有 ${(r.earlyRate * 100).toFixed(0)}% 的場次提早破城`)
}
// 5. 快的人要真的比較容易贏
