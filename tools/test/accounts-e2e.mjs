...
await p.locator('.start-btn').click({ force: true }); await p.click('.tabs button:nth-child(2)')   // 第一次使用
...
if (!signedUp) {
  await p.locator('.start-btn').click({ force: true }); await p.click('.tabs button:nth-child(1)')
  await p.fill('#em', TEACHER)
  await p.fill('#spw', TEACHER_PW)
  await p.click('button[type=submit]')
...
p.on('pageerror', (e) => bad.push('JS 例外：' + e))
await p.goto(BASE)
await p.locator('.start-btn').click({ force: true }); await p.click('.tabs button:nth-child(2)')
